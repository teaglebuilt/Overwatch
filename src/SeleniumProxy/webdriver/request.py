...
class InspectRequestsMixin:
    ...
    def wait_for_request(self, path, timeout=10):
        ...
        raise TimeoutException(
            'Timed out after {}s waiting for request {}'.format(timeout, path))
    ...
    def wait_for_response(self, path, timeout=10):
        ...
        raise TimeoutException(
            'Timed out after {}s waiting for response {}'.format(timeout, path))
...
class Request:
    ...
    def __init__(self, data, client):
        ...
        if data['response'] is not None:
            self.response = Response(
                self._data['id'], data['response'], client)
            # self.log.debug('response', extra={
    ...
    def body(self):
        if self._data.get('body') is None:
            self._data['body'] = self._client.get_request_body(
                self._data['id'])
...
class Response:
    ...
    def body(self):
        if self._data.get('body') is None:
            self._data['body'] = self._client.get_response_body(
                self._request_id)
